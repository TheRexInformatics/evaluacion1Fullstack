const STATUS_STYLES = {
  ...
  PROCESADO: "bg-emerald-50 text-emerald-700 border border-emerald-200",
};
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-3 mb-4">
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400"
          />
        </div>
        <select
          value={filtroEstado}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-100">
              ...
              <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Estado Saga</th>
          ...
          <tbody className="divide-y divide-slate-50">
            {pedidos.map((pedido) => (
              ...
              >
                <td className="px-5 py-3.5 font-mono font-semibold text-slate-700 text-xs">
                  #{pedido.id}
                </td>
                ...
                <td className="px-5 py-3.5 text-indigo-600 hover:text-indigo-700 text-xs font-medium">
                  Ver Detalle →
                </td>
              </tr>
            ))}
