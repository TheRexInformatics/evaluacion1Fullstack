...
/* ── Tabla principal ────────────────────────────────────────────────────── */
function TablaPedidos({ pedidos, loading, onEdit, onCancel }) {
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-gray-100">
            {["Pedido", "Cliente", "Ítems", "Total", "Fecha", "Estado Saga", "Acciones"].map((h) => (
        ...
        <tbody className="divide-y divide-gray-50">
          {loading ? (
            ...
            pedidos.map((p) => {
              const isCancellable = p.status === "PENDING" || p.status === "CONFIRMED";
              const isEditable    = p.status !== "CANCELLED";
              return (
                <tr key={p.id} className="hover:bg-gray-50 transition-colors duration-100">
                  ...
                  <td className="px-5 py-3.5 text-gray-700 font-medium max-w-40 truncate">{p.client}</td>
                  <td className="px-5 py-3.5 text-gray-500">{p.items}</td>
                  <td className="px-5 py-3.5 text-gray-800 font-semibold">${p.total.toLocaleString("es-CL")}</td>
                  <td className="px-5 py-3.5 text-gray-500 whitespace-nowrap">{p.fecha}</td>
                  <td className="px-5 py-3.5"><SagaBadge status={p.status} /></td>
...
/* ── Vista principal (Presenter raíz) ───────────────────────────────────── */
    ...
    <main className="flex-1 overflow-y-auto p-6 space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        <FiltrosPedidos
          ...
          onFilterFechaDesde={onFilterFechaDesde}
          onFilterFechaHasta={onFilterFechaHasta}
        />
        <TablaPedidos
          pedidos={pedidos}
          loading={loading}
      ...
      {/* Dialog confirmación cancelar (Saga) */}
      <ConfirmDialog
        show={!!confirmCancelId}
        title="Cancelar Pedido"
        message={`¿Estás seguro que deseas cancelar el pedido ${confirmCancelId}? Esta acción iniciará la transacción de compensación Saga y no se puede deshacer.`}
